...
ProxyCache.prototype._wrap = function (instance, method) {
  ...
  return function () {
    ...
    self.stats.incr('calls', 1, tags);
    self.stats.gauge('size', self.cache.length, tags);

    if (self._has(key)) {
      self.stats.incr('hit', 1, tags);
    ...
    args.push(function (err, result) {
      self.stats.timer('duration', new Date - start, tags);
...
ProxyCache.prototype._get = function (key) {
  ...
  return clone(get.call(cache, key));
};
